...
contract EproductsInfoItem{
    uint[] _timestamp; //保存电子产品流转过程中各个阶段的时间戳
    string[] _traceName; //保存电子产品流转过程各个阶段的用户名
    address[] _traceAddress; //保存电子产品流转过程各个阶段的用户地址信息（和用户一一对应）
    uint8[] _traceQuality; //保存电子产品流转过程中各个阶段的质量
    string _name; //电子产品名称
