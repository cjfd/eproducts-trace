...
const Farm = {
  ...
  data() {
    return {
      popup: false, // 创建电子产品弹窗
      eproductsList: [], // 电子产品信息
  }
  },
  computed: {
    // 当前页渲染数据
  ...
  template: `
    <div>
      <!-- 创建电子产品按钮 -->
      <el-button type="primary" @click="popup = true;">新建电子产品</el-button>
      ...
      <!-- 创建电子产品子组件 -->
      <CreateEproducts :dialogFormVisible="popup" @popup="handlePopup" @confirmPopup="formSubmit" />
    </div>
  `,
  mounted() {
    // 挂载时获取厂家数据
    ...
    <el-row style="height: 100%;" class="consumer">
      ...
      <el-col :span="16">
        <div class="grid-content bg-purple-light">
          ...
          <div v-if ="onSearch && eproductsDetail.length === 0" class="consumer-tip">该溯源码无对应信息，请确认后重新查询</div>

          <!-- 溯源查询成功后才展示 -->
          <div v-if ="onSearch && eproductsDetail.length">
            <!-- 溯源详情组件， user: 3 为消费者 -->
            <TraceDetail :eproducts-detail="eproductsDetail" :user="3" />
          </div>
        </div>
      </el-col>
      <el-col :span="1">
